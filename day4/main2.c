...
#define MAX_NUMBER_CARDS 203


static FILE *open_input_file_or_panic(int argc, const char **argv) {
    if (argc != 2) {
...
size_t find_matches_in_card(const char *card) {
    ...
    uint8_t winning_map[MAX_NUMBER_IN_WINNING_MAP] = {
        0}; // Use number to index into the map
    while (card[index] != '|') {
        ...
        assert(winning_number < MAX_NUMBER_IN_WINNING_MAP &&
               "Increase the MAX_NUMBER_IN_WINNING_MAP");
        winning_map[winning_number] = 1;

        while (isdigit(card[index])) {
            ++index; // advance to first non digit char
        }
        ++index; // skip one space so the pipe will be at index on end or a
                 // digit one the next number
    }
    ...
    while (card[index] != '\n' && card[index] != '\0') {
        ...
        assert(found_number < MAX_NUMBER_IN_WINNING_MAP &&
               "Increase the MAX_NUMBER_IN_WINNING_MAP");
...
int main(int argc, const char **argv) {
    ...
    size_t line_index;
    for (line_index = 0;
         (chars_read = getline(&line, &line_buffer_len, file)) != -1;
         ++line_index) {
        ...
        assert((line_index + found_matches) < MAX_NUMBER_CARDS &&
               "Increase MAX_NUMBER_CARDS");
        for (size_t copied_card_offset = 1; copied_card_offset <= found_matches;
             ++copied_card_offset) {
            card_count[line_index + copied_card_offset] +=
                card_count[line_index];
        }
    }
